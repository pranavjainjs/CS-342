...
int main(int argc, char * argv[])
{
    ...
    char buffer[1000]; // Buffer for storing the message
    ...
    while(1)
    {
        ...
        for(int j = 0; j < min(int(a.size()),1000); j++)
        {
            buff[i] = a[j];
            i++;
        }
        ...
        i++;
        buff[i] = ' '; i++;
        for(int j = 0; j < min(int(c.size()),1000); j++)
        {
            buff[i] = c[j];
            i++;
        }
        ...
        // sending to the server
        sendto(clientsocket,buff,strlen(buff),0,(struct sockaddr *)&server, sizeof(server));

        bzero(buffer, 1000);

        // receiving the answer from the server
        int bytesRead=recvfrom(clientsocket,buffer,1000,0,(struct sockaddr *)&server, &length);
