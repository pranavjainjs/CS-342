...
// Function to receive and reply messages from client
void receive_and_reply(int c_fd,struct sockaddr_in *client, map <ll,string> &port_client, int &number_of_clients)
{
	...
	// Function to receive messages from the client and reply to them
	while(1)
	{
		bzero(buffer,1500); 
		...
		if(buffer[0]=='1')
		{
			...
			cout << BOLDMAGENTA << "Message : ";
			cout << BOLDYELLOW << buffer+1 << RESET << endl;
			cout << BOLDMAGENTA << "Please enter your reply." << RESET << endl;

			bzero(buffer,1500);
			buffer[0]='2'; // Reply message

            string s; getline(cin, s); // Taking in the input
            for(int i = 1; i <= min(int(s.size()),1500); i++)
            {
                buffer[i] = s[i-1];
            }
	...
	cout << endl;
	cout << BOLDCYAN << port_client[port] << "'s connection has been closed" << endl;
	exit(0);
}
	...
	while(1)
	{
		...
		switch(status)
		{
			...
			case 0:
				...
				receive_and_reply(clientSocket,&client,port_client,number_of_clients); // this functions handles a client
				break;
			//parent process 
			default:
